...
import {UI} from "./ui.js";import {QuestSystem} from "./quests.js";
export class Game{
 constructor(canvas){
  ...
  this.world=new World();this.player=new Player(this);this.systems=new Systems(this);this.quests=new QuestSystem(this);this.ui=new UI(this);this.load();
 }
 onKey(e){
  ...
  if(k==="c"){this.ui.toggleCrafting();return}if(k==="m"){this.ui.toggleMap();return}if(k==="j"){this.ui.toggleQuests();return}if(k==="p"){this.ui.toggleMarketplace();return}if(k==="k"){this.ui.toggleDiscoveries();return}if(k==="l"){this.ui.toggleGuide();return}
 ...
 save(){localStorage.setItem("starRepublicSaveV2",JSON.stringify({player:this.player.serialize(),world:this.world.serialize(),farm:this.systems.farmSlots,quests:this.quests.serialize()}));this.ui.toast("Oyun kaydedildi")}
 load(){try{const raw=localStorage.getItem("starRepublicSaveV2");if(!raw)return;const s=JSON.parse(raw);this.player.restore(s.player);this.world.restore(s.world);if(Array.isArray(s.farm)&&s.farm.length===this.systems.farmSlots.length)this.systems.farmSlots=s.farm;if(s.quests)this.quests.restore(s.quests)}catch(e){console.warn("save load",e)}}
