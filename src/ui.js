...
renderHotbar(){
 ...
 renderCrafting(){
  ...
  const visible=RECIPES.filter(r=>r.id!=="astral_compass"||this.game.player.discoveries.bloom);
  let selected=visible[0],filter="Tümü";
  const categories=["Tümü",...new Set(visible.map(r=>r.category||"Temel"))];
  el.innerHTML='<div class="modal-card inventory-card crafting-card"><div class="inv-title"><h2>📖 Zanaat Kitabı</h2><span class="close">C / Kapat</span></div><p class="muted">Bir tarifi seç. Ürünün ne işe yaradığını gör, malzemelerini kontrol et ve tek tuşla üret.</p><div class="craft-tabs">'+categories.map(c=>'<button class="craft-tab '+(c==="Tümü"?"active":"")+'" data-cat="'+c+'">'+c+'</button>').join("")+'</div><div class="craft-layout"><div class="craft-recipes"><div class="craft-section-title">Tarifler</div><div id="craft-recipe-list"></div></div><div class="craft-workbench"><div id="craft-selected"></div><div class="craft-board-wrap"><div class="craft-grid" id="craft-grid"></div><div class="craft-arrow">➜</div><div class="craft-output" id="craft-output"></div></div><div id="craft-materials"></div><button id="craft-button" class="craft-button">ÜRET</button></div></div></div>';
  const drawList=()=>{
    const list=visible.filter(r=>filter==="Tümü"||(r.category||"Temel")===filter);
    if(!list.includes(selected))selected=list[0]||visible[0];
    el.querySelector("#craft-recipe-list").innerHTML=list.map(r=>'<button class="craft-recipe '+(canCraft(inv,r)?"ready":"")+' '+(r.id===selected.id?"chosen":"")+'" data-recipe="'+r.id+'"><span class="recipe-icon">'+this.icon(r.result)+'</span><b>'+r.name+'</b><small>'+(r.category||"Temel")+' · '+(canCraft(inv,r)?"Hazır":"Malzeme eksik")+'</small></button>').join("");
    el.querySelectorAll(".craft-recipe").forEach(x=>x.onclick=()=>{selected=visible.find(r=>r.id===x.dataset.recipe);draw();});
  };
  const draw=()=>{
    drawList();
    const p=selected.pattern||[];
    el.querySelector("#craft-grid").innerHTML=Array.from({length:9},(_,i)=>{const id=p[i]||"";return '<div class="craft-cell">'+(id?'<span>'+this.icon(id)+'</span><small>'+item(id).name+'</small>':"")+'</div>'}).join("");
    const info=item(selected.result);
    el.querySelector("#craft-output").innerHTML='<div class="craft-product-icon">'+this.icon(selected.result)+'</div><b>'+info.name+'</b><small>×'+selected.amount+'</small><em>'+info.rarity+'</em>';
    el.querySelector("#craft-selected").innerHTML='<div class="craft-detail"><div class="craft-hero-icon">'+this.icon(selected.result)+'</div><div><h3>'+selected.name+'</h3><p>'+selected.description+'</p><span class="muted">Sonuç: '+info.name+' · '+info.rarity+'</span></div></div>';
    el.querySelector("#craft-materials").innerHTML='<div class="material-title">Gerekli malzemeler</div>'+Object.entries(selected.ingredients).map(([id,n])=>'<span class="'+(inv.has(id,n)?"have":"missing")+'">'+this.icon(id)+' '+item(id).name+': '+inv.count(id)+'/'+n+'</span>').join("");
    const btn=el.querySelector("#craft-button");btn.disabled=!canCraft(inv,selected);btn.textContent=btn.disabled?"MALZEME EKSİK":"ÜRET";
  };
  el.querySelectorAll(".craft-tab").forEach(b=>b.onclick=()=>{filter=b.dataset.cat;el.querySelectorAll(".craft-tab").forEach(x=>x.classList.toggle("active",x===b));drawList();draw();});
  draw();
  el.onclick=e=>{if(e.target.classList.contains("close")||e.target===el)el.classList.add("hidden")};
  el.querySelector("#craft-button").onclick=()=>{if(craft(this.game,selected))draw();};
 }
 updateTutorial(){const el=this.$("tutorial"),txt=this.$("tutorial-text");if(!el||!txt)return;const p=this.game.player,w=this.game.world,q=this.game.quests;const steps=[["1 / 7","🌿 İlk kaynaklar","2 Dal + 2 Lif + 2 Taş topla.",(p.inventory.branch||0)>=2&&(p.inventory.fiber||0)>=2&&(p.inventory.stone||0)>=2,25],["2 / 7","🛠️ İlk alet","C ile İlk Yontu üret ve kuşan.",!!p.inventory.crude_tool,35],["3 / 7","🪓 Oduncu","Baltayı kuşan ve bir ağacı kes.",(p.inventory.hand_axe||0)>0&&(p.inventory.wood||0)>0,50],["4 / 7","🌱 İlk tarla","Bir tohum ek ve ilk ürünü hasat et.",!!p.inventory.wheat,60],["5 / 7","🏘️ Köyü tanı","Köyde bir NPC ile E kullanarak konuş.",Object.keys(q.done||{}).length>=1,75],["6 / 7","✦ İlk gizem","Astral Bloom'u keşfet ve 25 Yankıya ulaş.",!!p.discoveries.bloom&&p.resonance>=25,100],["7 / 7","⛏️ Derine in","Madende 3. derinliğe ulaş.",(w.mineDepth||0)>=3,150]];let step=Math.min(steps.length,p.tutorialStep||0);while(step<steps.length&&steps[step][3]){if(!p.tutorialRewards)p.tutorialRewards={};if(!p.tutorialRewards[step]){p.gold+=steps[step][4];p.gainXp(15);p.tutorialRewards[step]=true;this.toast("Eğitim tamamlandı: +"+steps[step][4]+" altın");this.game.save(true)}step++;}p.tutorialStep=step;if(step>=steps.length){txt.innerHTML="<b>🎉 Eğitim tamamlandı.</b><br>Kendi macerana devam et: keşfet, üret, sat ve yeni bölgeleri aç.";return}const x=steps[step];txt.innerHTML="<b>"+x[0]+" · "+x[1]+"</b><br>"+x[2]+"<br><span style='opacity:.65'>Ödül: "+x[4]+" altın</span>"}
